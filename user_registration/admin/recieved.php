...
if($DB->record_exists('lcl_individual_enrollment', array('registration_id'=>$registration_id))) {
        $user_details = $DB->get_record('lcl_individual_enrollment',array('registration_id'=> $registration_id));
        ...
        $email_template = $DB->get_record('lcl_email_template', array('type'=>1, 'template_type'=>5));
        ...
        $redirecting_url = $CFG->wwwroot.'/local/user_registration/home.php?id='.base64_encode($registration_id);
    }

    if($DB->record_exists('lcl_corporate_enrollment', array('registration_id'=>$registration_id))) {
        $user_details = $DB->get_record('lcl_corporate_enrollment',array('registration_id'=> $registration_id));
        ...
        $email_template = $DB->get_record('lcl_email_template', array('type'=>2, 'template_type'=>5));
        $subject = $email_template->subject;
        $msg = $email_template->template;
        $email_template_logo_address = $DB->get_record('lcl_logo_address', array('type'=>3));
        $logo = $email_template_logo_address->logo;
                            ...
                            <html>
                                ...
                                <body>
                                    <div class='main'>
                                        ...
                                        <div class='tablediv'>
                                            <div class='col-md-12' align='center'><h1><strong>(Recieved Letter)</strong></h1></div>
    ...
    echo json_encode(array('statustd'=> '<span class="text-danger">Recieved</span>', 'status' => '<span class="text-danger">Application Recieved, Please check your mail.</span>'));
}