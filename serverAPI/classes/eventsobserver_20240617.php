...
class local_user_registration_eventsobserver
{
    ...
    {
    	global $CFG;
        include($CFG->dirroot.'/local/user_registration/admin/qbk/apiCall.php');
        $restobj = new stdClass();
        $restobj->fullname = $event->other['fullname'];
        $restobj->description = strip_tags($_POST['summary_editor']['text']);
        ...
        $restobj->itemName = $event->other['shortname'];
        try {
            $APIcreateitem = createItem($restobj);
        } catch (Exception $e) {
