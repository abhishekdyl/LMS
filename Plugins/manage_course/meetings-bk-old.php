...
// $mform->addElement('editor', 'introeditor', 'texttt', array('rows' => 10), array('noclean' => true, 'context' => $context, 'subdirs' => true));
// $mform->setDefault('introeditor', array('text'=>'Default text!', 'format' => FORMAT_HTML));
// $mform->setType('introeditor', PARAM_RAW);

    
    $mform->addElement('editor','syllabus', 'Syllabus Override', 'wrap="virtual" rows="10" cols="10"');
    ...
    if (!empty($groupolddata->syllabus)) {
      $mform->setDefault('syllabus', array('text'=>$groupolddata->syllabus, 'format' => FORMAT_HTML));
    }else{
      $mform->setDefault('syllabus', array('text'=>$crscustomdata['syllabus'], 'format' => FORMAT_HTML));
    }
...
if ($mform->is_cancelled()) {
  ...
  // foreach ($_POST['endtime'] as $k=> $meetendtime) {
  //     if(strpos($meetendtime, " ")){
  //       $_POST['endtime'][$k] = strtotime($meetendtime);
  //     }
  // }
  //die;
// echo $CFG->dirroot . '/local/manage_course/emoji.php';

// require_once($CFG->dirroot . '/local/manage_course/emoji.php');
require_once($CFG->dirroot . '/local/manage_course/emoji_detector.php');

 // echo "<pre>";
// echo $_POST['syllabus']['text'];
// echo "<br>";
// $emoji = Emoji\detect_emoji($_POST['syllabus']['text']);

// print_r($emoji);

 // $text =emoji_unified_to_html();
        // echo $text;
        // echo emoji_html_to_unified($text);
  ...
  if (empty($formdata->name)) {
    ...
    if (!has_capability('moodle/course:changeidnumber', $coursecontext)) {
      ...
      $obj->syllabus = $formdata->syllabus['text'];
     ...
     // $obj->syllabus = htmlentities($formdata->syllabus,ENT_QUOTES, "UTF-8");
      ...
      if (!empty($obj->id)) {
        ...
        // $post_da = Emoji\detect_emoji($course_sections_data->post_data);
        $obj->post_data = json_encode($_POST);
      ...
      if( $request == 0 ){ 
        ...
        foreach ($allsectiondata as $skey => $course_sections_data) {
          ...
          $course_sections_data->post_data = json_decode($post_da);
