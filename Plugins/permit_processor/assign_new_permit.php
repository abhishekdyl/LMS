    ...
    <div class="wrapper-conete">
        ...
        <div class="progresstion-clip">
            <ul class="multi-step-bar">
                ...
                <li class="stepbtnnew current ">Step2 :Assign New Permit</li>
        ...
        <div class="header-title-btn w-100">
            ...
            <div class="left-col col-sm-6">
                <span>Assign New Permit</span>
                <p>Review selected students and assign new permit for them</p>
            </div>
            ...
            foreach ($_SESSION['usersid'] as $useid => $value) {
                ...
                if (empty($newuser_metaa['permit_number'][0])) {
                    array_push($nopermitusers, $useid);
                }
            }
            // echo "<pre>=====";
            $npusers = json_encode($nopermitusers);
            // echo "-----</pre>";
            ?>
            ...
            <div class="right-col col-sm-6">
                ...
                <?php
                if ($npusers != '[]') {
                    echo '<button class="bt-1" onclick="action_permit_assign_new(' . $npusers . ')">Assign Permit #</button>';
                } ?>
                <button class="bt-2 btnExport" id="btnExport">Export for Print</button>
                <button class="bt-3 btnNextpage" userid="<?php echo $npusers; ?>">Continue to Next Step</button>
        ...
        <div class="dex  table-responsive">
            <section id="table-section" class="license w-100">
                ...
                // //         array_push($usersdata,$newuser_metaa);
                // //         echo '++'.$newuser_metaa['first_name'][0];
                //         echo "<pre>--";
                ...
                <script type="text/javascript">
                    ...
                    $(".btnNextpage").click(function() {
                        var udata = $('#dtupgra').attr('dtupgrade');
                        var sessdata = $(this).attr('userid');
                        if (udata == '' && sessdata != '[]') {
                            // console.log('bbbbbbbbbbb');
                            if (confirm("Are you sure to skip the Assign permit.")) {
                                $(".stepbtnnew").addClass("active");
                                window.location.href = '/wp-admin/admin.php?page=upgradepermit';
                            }
                        } else {
                            // console.log('aaaaaaa');
                            $(".stepbtnnew").addClass("active");
                    ...
                    function action_permit_assign_new(id) {
                        console.log('arr---', id);
                        console.log('arr22---', jQuery.type(id));
                        // ids =  JSON.parse(id);
                        //alert(id);
                        jQuery.ajax({
                            type: "POST",
                    ...
                    class csvExport {
                        ...
                        static safeData(td) {
                            let data = td.textContent.trim().replace('New', '<br>').replace(/\s+/g, " ").replace(' <br> ', '\n');
