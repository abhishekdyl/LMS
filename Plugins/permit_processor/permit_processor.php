...
function add_permit_range() {	
    ...
    // delete multy permit number
    if(isset($_POST['submit'])){
        $permitlist = $_POST['select_all'];
        foreach ($permitlist as $permitid) {
            $sqlDel = "DELETE FROM " . $wpdb->prefix . "permit_number WHERE id='". $permitid ."'";
            $queryDel = $wpdb->query($sqlDel);
    ...
    <div class="row">
        <div class="col-md-8">
            <form method="post" action="" >
                        ...
                        <button type="submit" name="submit" id="delete_permit_all" class="btn btn-warning btn-md" style="margin-top: 2px; margin-bottom: 6px;">Delete Now</button>
            </form>
        </div>
    </div>
    </div>
