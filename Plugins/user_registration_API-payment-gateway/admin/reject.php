...
$redirecting_url = $CFG->wwwroot.'/local/user_registration/';
...
if($DB->record_exists('lcl_individual_enrollment', array('registration_id'=>$reg_id))) {
	...
	$email_template = $DB->get_record('lcl_email_template', array('type'=>1, 'template_type'=>3));
...
if($DB->record_exists('lcl_corporate_enrollment', array('registration_id'=>$reg_id))) {
	...
	$email_template = $DB->get_record('lcl_email_template', array('type'=>2, 'template_type'=>3));
	$msg = $email_template->template;
    $subject = $email_template->subject;
	$email_template_logo_address = $DB->get_record('lcl_logo_address', array('type'=>3));
	$logo = $email_template_logo_address->logo;
...
if(isset($_POST['reg_id'])){
	if($DB->record_exists('lcl_registration', array('id'=>$reg_id))) {
							...
							<html>
								...
								<body>
									<div class='main'>
										...
										<div class='tablediv'>
                                     		<div class='col-md-12' align='center'><h1><strong>(Rejection Letter)</strong></h1></div>
		...
		echo json_encode(array('statustd'=> '<span class="text-danger">Rejected</span>', 'status' => '<span class="text-danger">Application rejected, and will be notified to user</span>'));
