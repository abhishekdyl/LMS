...
$course_id = $_POST['course_id_priority']; 
$search_student = $_POST['search_student_priority']; 
$filter_order = $_POST['filter_order_priority']; 
$filter_status = $_POST['filter_status_priority']; 
$tutor_id  =$_POST['tutor_id_priority'];
...
$iTotalRecordsql ="
    ...
    INNER JOIN  {assign_subs_status} ass ON ass.userid = u.id AND ass.submissionid = asb.id AND ass.ispriority = 1
    ...
    INNER JOIN  {user_enrolments} ue ON ue.enrolid = e.id and ue.userid = u.id
     ...
     LEFT JOIN  {assign_subs_tutors} ast on ast.id = asat.tutor_id AND ast.active=1 AND ast.deleted_status =0
    {$searchQuerystring}
     AND asb.status='$submitted' AND ag.id IS NULL
";
$iTotalRecords = $DB->get_field_sql($iTotalRecordsql);
$orderQuery = array();
...
if($rowperpage != ''){
    if($columnName=='Courses_priority'){         
      $columnName = "c.id"; 
    }
    if($columnName=='Students_priority'){        
      $columnName = "u.id"; 
    }
    if($columnName=='Submission_Date_priority'){ 
      $columnName = "asb.timemodified"; 
    }
    if($columnName=='Assign_priority'){          
      $columnName = "asat.id"; 
    }
...
$sql = "
    ...
    INNER JOIN  {user_enrolments} ue ON ue.enrolid = e.id and ue.userid = u.id
     ...
     LEFT JOIN  {assign_subs_assign_tutors} asat ON asat.course_id = c.id AND asat.assignment_id = a.id AND asat.user_id = u.id
     LEFT JOIN  {assign_subs_tutors} ast on ast.id = asat.tutor_id AND ast.active=1 AND ast.deleted_status =0
    {$searchQuery} AND asb.status='$submitted' AND ag.id IS NULL {$orderQuery} LIMIT {$rowstart}, {$rowperpage}
";
...
$iTotalDisplayRecordsSql = "
    ...
    INNER JOIN  {user_enrolments} ue ON ue.enrolid = e.id and ue.userid = u.id
     ...
     LEFT JOIN  {assign_subs_assign_tutors} asat ON asat.course_id = c.id AND asat.assignment_id = a.id AND asat.user_id = u.id
     LEFT JOIN  {assign_subs_tutors} ast on ast.id = asat.tutor_id AND ast.active=1 AND ast.deleted_status =0
    {$searchQuery} AND asb.status='$submitted' AND ag.id IS NULL
 ";
$iTotalDisplayRecords = $DB->get_field_sql($iTotalDisplayRecordsSql);
...
$iPriorityMarkedSql = "
    ...
    INNER JOIN  {user_enrolments} ue ON ue.enrolid = e.id and ue.userid = u.id
     ...
     LEFT JOIN  {assign_subs_assign_tutors} asat ON asat.course_id = c.id AND asat.assignment_id = a.id AND asat.user_id = u.id
     LEFT JOIN  {assign_subs_tutors} ast on ast.id = asat.tutor_id AND ast.active=1 AND ast.deleted_status =0
     $searchQuery AND asb.status='$submitted' AND ag.id IS NOT NULL AND ast.id IS NOT NULL AND ag.timemodified > ?
  ";
$iPriorityMarked = $DB->get_field_sql($iPriorityMarkedSql, array(strtotime(date("d F Y"))));
...
foreach ($allforums as $key => $data) {
        ...
        $data->Courses_priority = "<span class='wrapon250' title='{$data->fullname}'>$data->fullname</span>";
        $data->Students_priority = $data->firstname." ".$data->lastname;
        $data->Submission_Date_priority = date("d/m/Y",$data->timemodified);
        ...
        if($get_assigned_tutorname = $DB->get_record_sql("SELECT id FROM {assign_subs_tutors} WHERE name='$data->tutorname'")){
            $selected = "selected";
            $select = '<select id="select_priority'.$count.'" class="form-control update_tutor_priority" data-courseid="'.$course_id.'" data-userid="'.$user_id.'" data-assignmentid="'.$assignment_id.'" style="display: none;">';
            foreach ($row_users as $key) {
              $select .= '<option value="'.$key->id.'" "'.$selected.'">'.$key->name.'</option>';
            }
            $select .= '</select><span id="span_none_priority'.$count.'">'.$data->tutorname.'&nbsp;&nbsp;&nbsp;<sup><i class="fa fa-edit" onclick="selectEnable_priority('.$count.')" id='.$count.'></i><sup></span>';
        } else {
            if($get_default_tutor_course_list = $DB->get_record_sql("SELECT tutor_id FROM {assign_subs_default_tutor} WHERE course_id=$course_id")){
                $get_assigned_tutor = $DB->get_record_sql("SELECT `name` FROM {assign_subs_tutors} WHERE active=1 AND deleted_status =0 AND id=$get_default_tutor_course_list->tutor_id");
                $None = $get_assigned_tutor->name."(Default)";
            } else {
                $None = "None";
            }
              $selected  ='';
              $select = '<select class="form-control update_tutor_priority" data-courseid="'.$course_id.'" data-userid="'.$user_id.'" data-assignmentid="'.$assignment_id.'">
        ...
        if(empty($data->tutorname)){
            ...
            if(!empty($onlinetext) && !empty($filename)){
                ...
                $Submission_priority = "<img src=".$src_arrow." alt='Some issue' width='20' /> <img src=".$src." alt='Some issue' width='20' /> ".$filename_update."";
            }else if(!empty($onlinetext)){
                $Submission_priority = "<img  src=".$src_arrow." alt='Some issue' width='20' /> <img src=".$src_txt." alt='Some issue' width='17' /> Onlinetext";
            }else {
                $Submission_priority = "<img  src=".$src_arrow." alt='Some issue' width='20' /> <img src=".$src_txt." alt='Some issue' width='17' /> Not found";
            }
        }else{
            ...
            if(!empty($onlinetext) && !empty($filename)){
                ...
                $Submission_priority = "<a target='_blank' style='text-decoration: none !important;' href='".$CFG->wwwroot."/mod/assign/view.php?id=".$cmid."&action=grader&userid=".$user_id."'/><img src=".$src_arrow." alt='Some issue' width='20' /> <img src=".$src." alt='Some issue' width='20' /> ".$filename_update."</a>";
            } else if(!empty($onlinetext)){
                $Submission_priority = "<a target='_blank' style='text-decoration: none !important;' href='".$CFG->wwwroot."/mod/assign/view.php?id=".$cmid."&action=grader&userid=".$user_id."'/><img  src=".$src_arrow." alt='Some issue' width='20' /> <img src=".$src_txt." alt='Some issue' width='17' /> Onlinetext</a>";
          ...
          $data->Status_priority = $status_chk;
          $data->Assign_priority =  $select;
          $data->Submission_priority = $Submission_priority;
          $allforums[$key] = $data;
          $count++;
  }
...
$response = array(
    ...
    "iPriorityMarked" => $iPriorityMarked,
