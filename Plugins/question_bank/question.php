...
$url = new moodle_url('/local/question_bank/question.php');
if ($id !== 0) {
    $url->param('id', $id);
}
if ($makecopy) {
...
$addpermission = has_capability('local/question_bank:addquestion', $categorycontext);
$approvepermission = has_capability('local/question_bank:approvequestion', $categorycontext);

if ($id) {
    $question->formoptions->canedit = $addpermission;//question_has_capability_on($question, 'edit');
...
if ($mform->is_cancelled()) {
    if ($inpopup) {
        ...
        $returnurl2 = $CFG->wwwroot.'/local/question_bank/index.php?courseid='.$courseid;
        redirect($returnurl2);
    }

} else if ($fromform = $mform->get_data()) {
    $fromform->status = 'draft';
    // echo "<pre>";
    // print_r($fromform);
    // echo "</pre>";
    // die;   
    ...
    $quizupdated = $DB->get_record('local_qbquestions',array('questionid'=>$question->id));
    if(empty($quizupdated)){
        $obj = new stdClass();
        $obj->questionid = $question->id;
        // $obj->status = $question->status;
        $obj->status = 'Sent for review';
        $obj->courseid = $courseid;
        $obj->coursecontextid = $courseContext->id;
        $obj->createdby = $question->createdby;
        $obj->createdtime = time();
        $aaa = $DB->insert_record('local_qbquestions',$obj);
    }else{
        $obj = new stdClass();
        $obj->id = $quizupdated->id;
        $obj->questionid = $question->id;
        $obj->status = $question->status;
        $obj->modifiedby = $question->modifiedby;
...
echo $OUTPUT->header();
if(!is_siteadmin()){
    echo '<style> #fitem_id_status{ display: none; } </style>';
}
