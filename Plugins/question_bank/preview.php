...
$questionreject = has_capability('local/question_bank:questionrejection', $categorycontext);
$reviewquiz = has_capability('local/question_bank:questionreviewed', $categorycontext);
...
// $previewdata['techinfo'] = print_collapsible_region_start('', 'techinfo', get_string('technicalinfo', 'question'),'core_question_preview_techinfo_collapsed', true, true, $OUTPUT->help_icon('technicalinfo', 'question'));
...
// $ddddd = helper::get_preview_extra_elements($question, $COURSE->id);
// echo "<pre>";
// print_r($ddddd);
// echo "------------------";
// echo $COURSE->id;
// print_r($comment);
// echo "</pre>";
// $comment = '<div class="db"><textarea name="content" rows="2" id="dlg-content-65b758259aa6c" aria-label="Add a comment..." class="fullwidth" style="color: grey;"></textarea></div>';
...
if(!is_siteadmin()){
    echo '<style> #previewcontrols{ display: none; } </style>';
}
    echo '<style> a.rview_btn{color:#0f6cbf; cursor:pointer;} </style>';
    if($approvercap && $locdata->status == 'reviewed'){
        echo ' <a class="rview_btn" id="approvebtn" >  Approve</a>';
    }
    if($questionreject && $locdata->status != 'review again' && $locdata->status == 'reviewed'){
        echo ' <a class="rview_btn" id="rejectbtn" >  Review again</a>';
    }
    if($reviewquiz && ($locdata->status == 'Sent for review' || $locdata->status == 'review again')){
        echo ' <a class="rview_btn" id="reviewedbtn">  Reviewed</a>';
    }
    if($reviewquiz && $locdata->status != 'pending' && $locdata->status != 'reviewed'){
        echo ' <a class="rview_btn" id="reviewreject">  Reject</a>';
    }
    echo '<input type="hidden" id="qid" value="'.$id.'">
    <br/><br/><a href="'.$CFG->wwwroot.'/local/question_bank/index.php?courseid='.$courseiid.'" class="btn btn-primary">  BACK</a>
    <div class="error"></div>';
echo $OUTPUT->footer();
?>
<script>
...
function myfunction(param){
    ...
    $.ajax({
        ...
        data: {action:'approve_reject',id:qid,status:param}, //sand data on url
...
$(document).ready(function(){
    $("#reviewedbtn").click(function(){
        ...
        $.ajax({
            type: 'post',
            url: '<?php echo $CFG->wwwroot; ?>/local/question_bank/ajax.php',
            data: {action:'reviewed',id:qid}, //sand data on url
            success: function (responseData) { //return responseData (anyname)
            var data = JSON.parse(responseData);
                console.log('formData',data);
    ...
    $("#reviewreject").click(function(){
        console.log('2222222');
        var qid = $("#qid").val();
        $.ajax({
            type: 'post',
            url: '<?php echo $CFG->wwwroot; ?>/local/question_bank/ajax.php',
            data: {action:'rejectreviewed',id:qid}, //sand data on url
            success: function (responseData) { //return responseData (anyname)
            var data = JSON.parse(responseData);
                console.log('formData',data);
