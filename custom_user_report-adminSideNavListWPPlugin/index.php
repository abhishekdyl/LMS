<?php
/**
  * Plugin Name: Custom User Report
  * Description: Filter Course Updated.
  * Version: 2.0.0
  * Author: LDS
...
function custom_user_report() {
    ...
    add_submenu_page('expreport', 'WSLCB New Export', 'WSLCB New Export', 'manage_options', 'wslcbexportnew', 'wslcbexport_new');

} 
function wslcbexport() {
    global $wpdb;
    echo $sql = 'SELECT u.*, um.meta_value as birthday FROM '. $wpdb->prefix .'users u JOIN '. $wpdb->prefix .'usermeta um ON u.ID = um.user_id AND um.meta_key LIKE "%date_of_birth%" WHERE u.ID >1 ORDER BY um.meta_value DESC LIMIT 200';
