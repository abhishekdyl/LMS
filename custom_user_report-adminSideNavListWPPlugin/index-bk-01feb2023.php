...
function custom_user_report() {
    ...
    add_submenu_page('expreport', 'Export User Report New', 'User Report New', 'manage_options', 'uexreport', 'exportUserReportnew');
...
if (isset($_POST['but_search'])) {
 ...
 <tr>
  <td><?php echo 78;  ?></td> 
 <td><?php echo 5465;  ?></td>
<?php $userId = $newuser->ID; 
           $Qccmplt = "SELECT * FROM wp_learndash_user_activity WHERE user_id='" . $userId . "' AND activity_type='quiz' AND activity_status=1 AND course_id ='".$eRecords->course_id."' ORDER BY activity_id DESC LIMIT  0,1";
...
<td><?php if(gettype($newuser_meta->height_ft_opt)=="integer"){ echo round($newuser_meta->height_ft_opt);}else{ echo $newuser_meta->height_ft_opt;}?></td> 
<td><?php if(gettype($newuser_meta->height_in_opt)=="integer"){ echo round($newuser_meta->height_in_opt);}else{ echo $newuser_meta->height_in_opt;} ?></td> 
...
<td><?php echo strtoupper($newuser->user_email);  ?></td>
...
<td><?php echo $newuser->user_login;  ?></td>
