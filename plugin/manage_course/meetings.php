...
class meeting_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('date_time_selector', 'startgroup', 'Start');
        ...
        if(!empty($groupolddata->startgroup)){
          $mform->setDefault('startgroup', $groupolddata->startgroup);
        }
        if(!empty($groupolddata)){
...
$groupolddata = null;
if($id){
  $sql = 'SELECT g.*, cg.teacher, cg.price, cg.startgroup, cg.availablespace FROM {groups} g LEFT JOIN {custome_groups} cg ON g.id = cg.groupid WHERE g.id = '.$id.'';
...
if($mform->is_cancelled()) {
  ...
  if ($formdata->id) {
    ...
    $formdata->id = groups_create_group($formdata, $editform, $editoroptions);    
  }
  $obj = new stdClass(); 
  ...
  $obj->startgroup = $formdata->startgroup;
  if(!empty($obj->id)){
...
// --------------------------------------
    ...
    $sectdata = course_create_section($courseorid, $position = 0, $skipcheck = false);
    $sectionid = $sectdata->section;
    //echo "<pre>";
    $course_sections_data = $DB->get_record('custome_groups',array('id'=>$customegroups));
    $course_sections_data->starttime = $_POST['starttime'];
    $course_sections_data->endtime = $_POST['endtime'];
    $wpurl = get_config('local_coursesync','wpurl');
    //print_r($course_sections_data);
    //echo json_encode($course_sections_data);
    $curl = curl_init();
    ...
    CURLOPT_POSTFIELDS =>json_encode($course_sections_data),
  ...
  if(!empty($moduleid)){
    ...
    foreach ($cmidarr as $value) {
      ...
      // zoom_delete_instance($value->zid);
    }
  ...
  if(is_array($_POST['starttime'])){
    for ($i=0; $i < sizeof($_POST['starttime']); $i++) { 
      ...
      $zooml->host_id = zoom_get_user_id();
      ...
      $response = add_moduleinfo($zooml, $coursedata);
    }
  }
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form>  
            ...
            <input type="button" class="close" style="    background-color: #c23a34 !important; opacity: 100;" value="Cancel">
