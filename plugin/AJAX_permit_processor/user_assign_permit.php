...
?>
    ...
    <div class="wrapper-conete">
                ...
                <div class="col-md-6 p-0">
                    <div class="row">
                        <div class="col-md-12 p-0">
                            <div class="title">
                                <h3 class="title_name">Student List</h3> <span class="bg-warning p-2 badge rounded-pill"> 200 student</span>
                            </div>
                        </div>
                        <div class="col-md-12 p-0">
        ...
        <div class="process_list mt-1 mb-2"> <span id="totalselecteds"> <?php echo sizeof($_SESSION["usersid"]); ?></span>/200 Add selected users to processing list: </div>


        <?php
        $btnrec_query = "SELECT * FROM wp_learndash_user_activity WHERE activity_type='quiz' AND activity_status=1 GROUP BY user_id ORDER BY activity_id DESC LIMIT 200";
        ...
        <div class="table-responsive">
            <table class="table table-bordered mt-5" id="example">
                ...
                if (isset($_POST['but_search'])) {
                    ...
                    // $rec_query = "SELECT * FROM `wp_learndash_user_activity` WHERE `activity_status` = 1 AND `activity_type` ='quiz' AND `course_id` = 900 ORDER BY `wp_learndash_user_activity`. `activity_id` DESC LIMIT 200";
                    // $rec_query = "SELECT * FROM wp_learndash_user_activity WHERE activity_type='quiz' AND activity_status=1 GROUP BY user_id ORDER BY activity_id DESC LIMIT 200";
                    $rec_query = "SELECT * FROM wp_learndash_user_activity WHERE activity_type='quiz'  AND activity_status=1 " . $datefltr . "" . $complete . " GROUP BY user_id ORDER BY activity_id DESC LIMIT 200";
                    ...
                    if ($permit == 1) {
                        ...
                        if ($std_class == 1) {
                            $rec_query1 = "SELECT * FROM wp_learndash_user_activity WHERE activity_type='quiz'  AND activity_status = 1 " . $datefltr . "" . $complete . " GROUP BY user_id ORDER BY activity_id DESC LIMIT 200";
    ...
    <input type="text" id="totalselected" value="<?php echo sizeof($_SESSION["usersid"]); ?>" />
    ...
    <script>
        ...
        $(".btnNextpage").click(function() {
            ...
            if (totaladded > 0) {
                ...
                window.location.href = 'https://staging-soqorura.temp312.kinsta.cloud/wp-admin/admin.php?page=assignewpermit';
            } else {
        ...
        $(document).ready(function() {
            ...
            $('#example').DataTable({
                ...
                //     {
                //         text: 'Upgrade Permit',
                //         className:'reassign-new',
                //         action: function ( e, dt, node, config ) {
                //             var arr = new Array();
                //             $('body #example tbody').find('input[type="checkbox"]').each(function(){
                //                 if($(this).is(":checked")){
                //                     if($(this).closest('tr').find('.btn-yellow').hasClass('btn-yellow')){
                //                         arr.push(parseInt($(this).val()));							
                //                     }
                //                 }
                //             // console.log('checkbox :',$(this));
                //             });
                //             if(arr.length){
                //                 action_permit_reassign(arr);

                //             }else{
                //                 alert("Please Select Atleast On Reassign Records");
                //             }
                //         // console.log('arrr',arr);
                //         }
                //     }
    ...
    <script type='text/javascript'>
        jQuery(document).ready(function() {
            // var countdata =  $('#example').DataTable().rows().count();
            // $('.process_list').html( '1/'+countdata+' Add selected users to processing list:');
            ...
            function exportTableToCSV($table, filename) {
                ...
                function grabRow(i, row) {
                    ...
                    var $row = $(row);
                    // var total_check_count=$('#example').find('input[type="checkbox"]').is(':checked').length();
                    // var total_count=$('#example').find('input[type="checkbox"]').length();
                    // console.log('i: ',i);
                    // //if($row.find('td')){
                    //     if(total_count != total_check_count ){
