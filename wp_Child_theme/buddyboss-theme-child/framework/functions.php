...
function moodle_course_section(){
    ...
    if($moodle_courses_enable == '0'):
        ...
        foreach($custom_repeater_item as $item_key => $item_value) {
            ...
            $group_syllabus = isset($item_value['group_syllabus']) ? $item_value['group_syllabus'] : '';
            if ($moodlegroupid == '0'){
                continue;
            }
            ...
            if (!empty($moodlegroupid)) {
                ...
                // LDS custom code
                $studentsenrolleddata = $wpdb->get_results("SELECT woi.*
                FROM {$wpdb->prefix}woocommerce_order_itemmeta as woim,
                     {$wpdb->prefix}woocommerce_order_itemmeta as woim2,
                     {$wpdb->prefix}woocommerce_order_items as woi,
                     {$wpdb->prefix}posts as p
                ...
                // echo "<pre>";
                $array_unique_user = array();

                if($studentsenrolleddata) {
                    foreach($studentsenrolleddata as $data) {

                        $sql = "SELECT *
                        FROM {$wpdb->prefix}woocommerce_order_itemmeta
                        WHERE order_item_id = ".$data->order_item_id." AND meta_key='Student Name'";
                        $itemmeta_data = $wpdb->get_results($sql);
                        if(empty($itemmeta_data)){
                            $parent_order_item_id = $data->order_item_id;
                            $parent_order_id = $data->order_id;
                            $order = wc_get_order($parent_order_id);
                            $order_encodeed = json_decode($order);
                            
                        }else{
                            $child_order_item_id = $data->order_item_id;
                            foreach($itemmeta_data as $datanew){
                                $sql_user = "SELECT DISTINCT(`ID`)
                                FROM {$wpdb->users}
                                WHERE user_login = '$datanew->meta_value'";
                                $chk_user_exists = $wpdb->get_results($sql_user);
                                foreach($chk_user_exists as $usernew){
                                    array_push($array_unique_user,$usernew->ID);
                                }
                            } 
                        }    
                    }
                ...
                // echo "<pre>";
                // print_r(array_unique($array_unique_user));
                // print_r($studentsenrolled);
                $studentsenrolled = count(array_unique($array_unique_user));
                // echo "Total Unique Count Child :".$total_unique_count;
                // echo "</pre>";
                // LDS End
            }
            ...
            if(sizeof($directfrommoodle) > 0){
                    ...
                    if(sizeof($dday) == 1){
                        ...
                        switch (sizeof($filtereddates)) {
                            ...
                            case 1:
                                ...
                                break;
                            case 2:
                                $course_subscription .= 'Two Meetings Every Week';
                                break;
                            case 3:
                                $course_subscription .= 'Three Meetings Every Week';
                                break;
                            case 4:
                                $course_subscription .= 'Four Meetings Every Week';
                                break;
                            case 5:
                                $course_subscription .= 'Five Meetings Every Week';
                                break;
                            case 6:
                                $course_subscription .= 'Meetings Daily';
                                break;
                            default:
                ...
                $section_data[] = array(
                   ...
                   // 'meet_startdate' => $meetdate[0],
                    ...
                    'course_syllabus' => $group_syllabus,
        ...
        $html .= '<div id="meetingbyzone"><table class="moodle-course-section-list table-responsive" id="mcslist" ><thead>';
        $html .= '<tr><th><strong><img src="' . get_theme_root_uri() . '/buddyboss-theme-child/framework/assets/images/4.png' . '" alt="" class="course-img">Live 
        Classes</strong></th>';
        ...
        foreach ($section_data as $data) {
            ...
            if($displaytype !=0 ){
                if($displaytype == 2){
                    ...
                    $html .= '<br><a href="#course-calendar-modal" class="calendar-link link" data-toggle="modal" data-id="' . $post->ID . '" data-count="' . $data['meet_item_key'] . '" data-value="' . $data['course_calendar'] .'" data-zone="' . $usertimezone .'">View All Meetings</a></span></td>';
                    $count++;
                } else {
                    $html .= '<td><span class="calendar-desc "> '.(($data['meet_durationweak'] == 0)?"":$data['meet_durationweak']." Weeks <br>" ).'Starts: '. date('M j, Y', $data['meet_startdate']).'<br>Ends: '.date('M j, Y', $data['meet_enddate']).'<br><a href="#course-calendar-modal" class="calendar-link link" data-toggle="modal" data-id="' . $post->ID . '" data-value="' . $data['course_calendar'] . '" data-count="' . $data['meet_item_key'] . '" data-zone="' . $usertimezone .'" >View All Meetings </a></span></td>';
                    $count++;
                }
            }
            ...
            if($displaytype !=0 ){
                $html .= '<td><span class="syllabus-data"><a href="#course-syllabus-modal" class="link syllabus-link" class="btn btn-primary" data-toggle="modal" data-target="#course-syllabus-modal" data-id="'.$data['course_syllabus'].'">Syllabus</a></span></td>';
            }
